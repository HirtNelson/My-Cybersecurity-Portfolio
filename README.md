Welcome! This repository contains my technical write-ups and documentation for various CTF machines and security 

# 🛡️ Nelson Hirt | Cybersecurity Portfolio

Welcome to my cybersecurity repository. This space serves as a technical log where I document my methodology, toolsets, and the creative paths taken to solve complex security challenges and CTFs.

---

## 🚩 TryHackMe Write-ups

Explore my step-by-step walkthroughs, ranging from initial reconnaissance to full system compromise.

| Room | Difficulty | Focus | Write-up |
| :--- | :---: | :--- | :---: |
| **Hammer** | 🟢 Easy | Web / Logic Flaws | [📂 View](./THM-Writeups/Hammer/README.md) |
| **Skynet** | 🟡 Medium | SMB / CMS Exploitation | [📂 View](./THM-Writeups/Skynet/README.md) |
| **Expose** | 🟢 Easy | SQLi / Fuzzing | [📂 View](./THM-Writeups/Expose/README.md) |
| **ToolsRus** | 🟢 Easy | Tomcat / Hydra | [📂 View](./THM-Writeups/ToolsRus/README.md) |
| **Basic Pentesting** | 🟢 Easy | Brute Force / PrivEsc | [📂 View](./THM-Writeups/Basic_Pentesting/README.md) |
  **RootMe**   | 🟢 Easy  | Insecure file upload | [📂 View](./THM-Writeups/RootMe/README.md) |

---

## 🛠️ Toolbelt & Skills
I frequently utilize the following technologies in my write-ups:

![Kali Linux](https://img.shields.io/badge/OS-Kali_Linux-blue?style=flat-square&logo=kali-linux)
![Nmap](https://img.shields.io/badge/Recon-Nmap-red?style=flat-square)
![Metasploit](https://img.shields.io/badge/Exploitation-Metasploit-blue?style=flat-square)
![Burp Suite](https://img.shields.io/badge/Proxy-Burp_Suite-orange?style=flat-square)
![Wireshark](https://img.shields.io/badge/Analysis-Wireshark-blue?style=flat-square)

---

### 📫 Connect with me:
[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/SEU-PERFIL)

*Developed by **Nelson Hirt** — 2026*